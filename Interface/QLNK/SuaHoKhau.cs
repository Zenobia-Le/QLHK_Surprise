...
namespace QLNK
{
    public partial class SuaHoKhau : MaterialForm
    {
        ...
        private readonly string[] _itemHoKhau;
        private bool _flagSave;
        public SuaHoKhau(string[] itemHoKhau)
        {
            InitializeComponent();
            _itemHoKhau = itemHoKhau;
        ...
        private void btnSuaHoKhau_Click(object sender, EventArgs e)
        {
                  ...
                  diaChi.Equals("")     || khuVuc.Equals("")))
            {
                ...
                _flagSave = true;
                if (!_sourceCode.UpdateHoKhau(maHoKhau, tenChuHo, cmndChuHo, khuVuc, diaChi, ngayLap)) return;
                Close();
        ...
        private void SuaHoKhau_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            if ((maHoKhau.Equals(_itemHoKhau[0]) && tenChuHo.Equals(_itemHoKhau[1]) && cmndChuHo.Equals(_itemHoKhau[2]) &&
                 khuVuc.Equals(_itemHoKhau[3]) && diaChi.Equals(_itemHoKhau[4]) && ngayLap.Equals(_itemHoKhau[5])))
                return;
            ...
            switch (dialogResult)
            {
                case DialogResult.Yes:
                    if (!(maHoKhau.Equals("")   || tenChuHo.Equals("")  || cmndChuHo.Equals("") ||
                          ...
                          diaChi.Equals("")     || khuVuc.Equals("")))
                    {
                        if (_sourceCode.UpdateHoKhau(maHoKhau, tenChuHo, cmndChuHo, khuVuc, diaChi, ngayLap))
                        {
                            MessageBox.Show(_sourceCode.SuccessUpdate);
                        }
                    }
